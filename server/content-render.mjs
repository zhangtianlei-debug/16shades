...
export function wikiPage(article, all, options) {
  ...
  const examples=(article.relationshipExamples??[]).filter(example=>example?.id&&example?.label?.[locale]);
  const exampleSection=examples.length?`<section><h2>${locale==='zh'?'相关关系实例':'Related relationship examples'}</h2><ul>${examples.map(example=>`<li><a href="${localizedPath(`/relationships/${example.id}`,locale)}">${escapeHtml(example.label[locale])}</a></li>`).join('')}</ul></section>`:'';
  const sourceSection=(article.sources??[]).length?`<section><h2>${locale==='zh'?'参考来源':'References'}</h2><ul>${article.sources.map(source=>`<li><a href="${escapeHtml(target(source.url,locale)??'#')}">${escapeHtml(typeof source.title === 'string' ? source.title : source.title[locale])}</a><p class="meta">${escapeHtml(source.authors)} · ${source.year} · ${escapeHtml(source.kind[locale])}</p>${source.open_access_url?` · <a href="${escapeHtml(target(source.open_access_url,locale)??'#')}">${locale==='zh'?'开放全文':'Open-access text'}</a>`:''}${source.findings?.[locale]?.length?`<p>${escapeHtml(source.findings[locale].join(' '))}</p>`:''}${source.limits?.[locale]?.length?`<p>${escapeHtml(source.limits[locale].join(' '))}</p>`:''}</li>`).join('')}</ul></section>`:'';
  const body=`<nav aria-label="${c.directory}"><a href="${localizedPath('/knowledge',locale)}">${c.wiki}</a></nav><article><p class="eyebrow">${escapeHtml(article.kind[locale])}</p><h1>${escapeHtml(article.title[locale])}</h1><p class="lead">${escapeHtml(article.summary[locale])}</p><p class="meta">${c.source}: ${c.brand} · ${c.updated} <time datetime="${article.updatedAt}">${article.updatedAt}</time></p><div class="prose">${markdown(article.body[locale],locale)}${exampleSection}${sourceSection}</div></article>${related.length?`<section class="related"><h2>${c.related}</h2><div class="cards">${related.map(item=>card(item,locale,`/knowledge/${item.slug}`)).join('')}</div></section>`:''}`;
